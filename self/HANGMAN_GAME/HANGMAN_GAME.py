...
def get_file_path():
    ...
    legal_path = os.path.exists(file_path)
    while not legal_path:  # checking if file path is legal
        file_path = input("Please Enter a valid path")
        legal_path = os.path.exists(file_path)
    return file_path
...
def hangman(secret_word):
    """
    :param secret_word: secret word the user in trying to guess
    ...
    while turn_counter <= MAX_TRIES and not win:
        ...
        while not check_valid_input(guess, old_letters_guessed):
